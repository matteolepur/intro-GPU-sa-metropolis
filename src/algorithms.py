...
class Metropolis(Algorithm):
    ...
    def step(self, x, W, Y, beta, energy, wx, idx, rand):
        # Compute the new energy value if we would update x^(t)
        energy_next, wx_next = utils.compute_energy_diff(wx, x, idx, W, Y)

        # Compute the acceptance probability
        a = torch.exp(-beta * (energy_next - energy)).clamp(max=1.0)
    ...
    def step_batch(self, x_batch, W, Y, beta, energy_batch, wx, idx_batch, rand_batch):
        # Compute the new energy value if we would update x^(t)
        energy_next, wx_next = utils.compute_energy_diff_batch(wx, x_batch, idx_batch, W, Y)

        # Compute the acceptance probability (batch-wise)
        a = torch.exp(-beta * (energy_next - energy_batch)).clamp(max=1.0)

        # Which batches pass the acceptance test?
        accepted_batches = rand_batch < a

        # Update x^(t) for the batches that pass the acceptance test
        x_batch[idx_batch[accepted_batches], accepted_batches.nonzero().squeeze(-1)] *= -1

        # Update the energy value for the batches that pass the acceptance test
        energy_batch[accepted_batches] = energy_next[accepted_batches]

        # Update Wx^(t) value for the batches that pass the acceptance test
        wx[:, accepted_batches] = wx_next[:, accepted_batches]
